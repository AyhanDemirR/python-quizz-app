...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        self.master.title("Quiz Uygulaması")
        ...
        self.mainTitle = Label(f, text="Genel Kültür Quiz", fg="red" ,bg="azure", font=("Helvetica", 30, "bold italic")).place(
            x=400, y=200)
        self.regi = Button(f, text="Kayıt Ol", width=20, height=5, fg="royalblue4", bg="lavender",
                           font=("Helvetica", 10, "bold italic"), command=self.c_reg)
        self.login = Button(f, text="Giriş Yap", width=20, height=5, fg="royalblue4", bg="lavender",
...
class Register:
    ...
    def __init__(self, master):
        ...
        self.master = master
        self.master.geometry("1300x700+0+0")
        self.master.title("Gene Kültü Quiz - Kayıt Ol")
        ...
        self.mainTitle = Label(f1, text="Kayıt Ol", bg="azure",
                               font=("Helvetica", 30, "bold italic underline")).place(x=50, y=10)
        self.name = Label(f1, text="Ad : ", bg="azure", font=("Times New Roman", 10))
        self.lname = Label(f1, text="Soyad : ", bg="azure", font=("Times New Roman", 10))
        self.email = Label(f1, text="Email : ", bg="azure", font=("Times New Roman", 10))
        self.uname = Label(f1, text="Kullanıcı Adı :", bg="azure", font=("Times New Roman", 10))
        self.pw = Label(f1, text="Parola : ", bg="azure", font=("Times New Roman", 10))
        ...
        self.submit = Button(f1, text="Onayla", width=20, height=5, fg="royalblue4", bg="lavender",
                             font=("Helvetica", 10, "bold italic"), command=self.c_submit)
        self.cancel = Button(f1, text="İptal", width=20, height=5, fg="royalblue4", bg="lavender",
                             font=("Helvetica", 10, "bold italic"), command=self.c_cancel)

        self.checkB = Checkbutton(f1, text='Parolayı Göster', bg="azure", fg="royalblue4",
    ...
    def check(self, l1):
        ...
        for i in range(5):
            ht = ht + 50
            if len(l1[i]) == 0:
                self.l = Label(f1, text="! Bu Alanı Boş Bırakamazsınız", fg='red', bg="azure")
                self.l.place(x=400, y=ht)
            else:
                self.l = Label(f1, text="! Bu Alanı Boş Bırakamazsınız", bg="azure", fg="azure")
                self.l.place(x=400, y=ht)
                f = f + 1
        if l1[2].find("@") == -1 and l1[2].find(".") == -1 and len(l1[2]) != 0:
            self.l = Label(f1, text="! Lütfen Geçerli Bir E-posta giriniz.", bg="azure", fg="red")
            self.l.place(x=400, y=200)
            s = 1
        else:
            if (len(l1[2]) > 0):
                self.l = Label(f1, text="! Lütfen Geçerli Bir E-posta giriniz.", bg="azure", fg="azure")
                self.l.place(x=400, y=200)
        if len(l1[4]) < 8 and len(l1[4]) != 0:
            self.l = Label(f1, text="! Parola 8 karakterden az olamaz", bg="azure", fg="red")
            self.l.place(x=400, y=300)
        else:
            if (len(l1[4]) > 0):
                self.l = Label(f1, text="! Parola 8 karakterden az olamaz", bg="azure", fg="azure")
                self.l.place(x=400, y=300)
    ...
    def c_submit(self):
        ...
        if c == 1:
            ...
            row = cursor.fetchone()
            if row is not None:
                messagebox.showwarning("Hata", "Kullanıcı adı alınmış, farklı bir kullancı adı giriniz!")
                mReg.destroy()
            else:
                try:
...
class Login:
    ...
    def __init__(self, master):
        ...
        self.l1 = Label(f2, text="Kullanıcı Adı: ", bg="azure", font=("Times New Roman", 20))
        self.e1 = Entry(f2, width=30)
        self.l2 = Label(f2, text="Parola: ", bg="azure", font=("Times New Roman", 20))
        self.e2 = Entry(f2, width=30, show="*")
        self.b1 = Button(f2, text="Giriş Yap", width=15, height=3, fg="royalblue4", bg="lavender",
                         font=("Helvetica", 10, "bold italic"), command=self.clicked)
        self.b2 = Button(f2, text="İptal", width=15, height=3, fg="royalblue4", bg="lavender",
                         font=("Helvetica", 10, "bold italic"), command=self.cancelLogin)

        self.var = IntVar()
        self.checkB = Checkbutton(f2, text='Parolayı Göster', bg="azure", fg="royalblue4",
                                  font=("Helvetica", 10, "bold italic"), variable=self.var, onvalue=1,
                                  offvalue=0, command=self.Showpasswd)
...
class Account:
    ...
    def __init__(self, master, u):
        ...
        cursor.close()
        conn.close()
        self.greet = Label(f3, text="Hey " + u + ", Hoşgeldin!", bg="azure",
                           font=("Helvetica", 30, "bold italic")).place(x=400, y=200)
        self.lastScore = Label(f3, text="Son Quiz Skoru = " + str(self.prevScore[0]), bg="azure",
                               font=("Helvetica", 30, "bold italic")).place(x=400, y=300)
        self.takeQuiz = Button(f3, text="Quize Katıl", width=20, height=5, fg="royalblue4", bg="lavender",
                               font=("Helvetica", 10, "bold italic"), command=self.goinside)
        self.takeQuiz.place(x=500, y=400)
        self.logout = Button(f3, text="Çıkış Yap", width=20, height=5, fg="royalblue4", bg="lavender",
...
class Quiz:
    def __init__(self, master, u):
        ...
        self.master = master
        self.master.geometry("1300x700+0+0")
        self.master.title("Genel Kültür Quiz - Kayıt Ol")
        self.master.config(bg="azure")
        global f1
        f = Frame(self.master, height=1080, width=1920, bg="azure", relief="ridge", bd=20)
        conn = MySQLdb.connect(host='localhost', database='quizdb', user='root', password='root')
        ...
        self.Back = Button(f, text="<- Geri", width=15, height=3, fg="royalblue4", bg="snow2",
                           font=("Helvetica", 10, "bold italic"), command=self.back).place(x=100, y=325)
        self.Next = Button(f, text="İleri ->", width=15, height=3, fg="royalblue4", bg="snow2",
                           font=("Helvetica", 10, "bold italic"), command=self.next).place(x=250, y=325)
        self.submit = Button(f, text="Onayla", width=34, height=2, fg="ghost white", bg="DeepSkyBlue2",
    ...
    def next(self):
        ...
        if self.qno >= len(questions):
            self.qno -= 1
            messagebox.showwarning("Dikkat", "Quizin Sonuna Geldiniz, Onayla Butonuna Basınız")
        else:
    ...
    def back(self):
        ...
        if self.qno < 0:
            self.qno += 1
            messagebox.showerror("Hata", "Başlangıçtasınız!!!")
        else:
            self.display_q(self.qno)
            c = l1[self.qno]
    ...
    def Submit(self):
        ...
        for i in range(10):
            if (l1[i] == 0):
                x += 1
        if (x > 0 and x != 1):
            y = messagebox.askyesno("Hata", + str(
                x) + " Soruyu Cevaplamadınız, Onaylamak istediğinize emin misiniz?, Seçimlerinizi değiştiremeyeceksiniz.")
        elif (x == 1):
            y = messagebox.askyesno("Hata", + str(
                x) + " Soruyu Cevaplamadınız, Onaylamak istediğinize emin misiniz?, Seçimlerinizi değiştiremeyeceksiniz.")
        if (y == True or x == 0):
        ...
        messagebox.showinfo("Skor", "Skorunuz: " + str(s) + "/10")
        mQuiz.destroy()
